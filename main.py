...
try:
    ...
    print("⚠ Arabic/Kurdish text shaping libraries not found. Install with: pip install arabic-reshaper python-bidi")
...
class TextToImageBot:
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self.text_to_image))
    
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle the /start command"""
        welcome_message = (
            "🎨 **Welcome to Text to Image Bot!**\n\n"
            "I can create two types of images:\n\n"
            "**1. Document Templates 📋**\n"
            "Send exactly 6 lines in this format:\n"
            "```\n"
            "34 DN 7567\n"
            "1414\n"
            "8/5/2025 1 PM\n"
            ...
            "محمد خلف\n"
            "```\n\n"
            "**2. Regular Text Images 📝**\n"
            "Send any other text for simple image conversion\n\n"
            "**Features:**\n"
            "• Arabic & English text support\n"
            "• Document templates with license plates\n"
            "• Clean, readable fonts\n"
            "• High quality PNG output\n\n"
            "Try sending the example format above! 🚛"
        )
    ...
    def create_document_template(self) -> Image:
        ...
        try:
            ...
            logger.warning("No template image found, creating fallback template")
    ...
    def create_fallback_template(self) -> Image:
        ...
        draw.rectangle([50, 50, 300, 150], fill=(255, 255, 255), outline=(200, 50, 50), width=4)
        
        # Draw TR license plate (top right - blue)
        draw.rectangle([900, 50, 1200, 120], fill=(70, 130, 180), outline=(0, 0, 0), width=2)
        draw.text([910, 60], "TR", fill=(255, 255, 255), font=self.get_font(60))
        
        # Draw name box (middle left - light blue)
        draw.rectangle([50, 200, 600, 280], fill=(173, 216, 230), outline=(100, 100, 100), width=2)
        
        # Draw date/time box (middle right - light pink)
        draw.rectangle([650, 200, 1200, 280], fill=(255, 182, 193), outline=(100, 100, 100), width=2)
        
        # Draw truck
        truck_y = 350
        # Truck cab (blue)
        draw.rectangle([50, truck_y, 200, truck_y + 150], fill=(70, 130, 180), outline=(0, 0, 0), width=2)
        # Truck trailer (light gray)
        draw.rectangle([200, truck_y + 30, 700, truck_y + 120], fill=(220, 220, 220), outline=(0, 0, 0), width=2)
        
        # IRQ license plate on truck (yellow)
        draw.rectangle([750, truck_y + 90, 1050, truck_y + 140], fill=(255, 215, 0), outline=(0, 0, 0), width=2)
        draw.rectangle([750, truck_y + 90, 790, truck_y + 140], fill=(255, 69, 0), outline=(0, 0, 0), width=1)
        draw.text([755, truck_y + 105], "IRQ", fill=(0, 0, 0), font=self.get_font(20))
        draw.text([755, truck_y + 120], "KR", fill=(0, 0, 0), font=self.get_font(18))
            ...
            for font_path in fonts_to_try:
                try:
                    ...
                    if "Rabar" in font_path:
                        logger.info(f"Successfully loaded Kurdish font: {font_path} (size: {size})")
                    return font
                except (OSError, IOError) as e:
                    # Only log Kurdish font loading issues for debugging
                    if "Rabar" in font_path:
                        logger.warning(f"Could not load Kurdish font {font_path}: {e}")
                    continue
    ...
    def shape_kurdish_text(self, text: str) -> str:
        ...
        try:
            ...
            if ARABIC_SUPPORT:
                ...
                logger.info(f"Shaped Kurdish text: '{text}' -> '{display_text}'")
                return display_text
            else:
                logger.warning("Arabic reshaping not available, text may not display correctly")
                return text
    ...
    def create_document_image(self, text: str) -> io.BytesIO:
        ...
        try:
            ...
            font_large = self.get_font(int(120 * min(scale_x, scale_y)))  # Adjusted to match reference image
            bbox = draw.textbbox((0, 0), data['doc_number'], font=font_large)
            text_width = bbox[2] - bbox[0]
            text_height = bbox[3] - bbox[1]
            # Center in the red-bordered box
            x = int((img_width * 0.215) - (text_width // 2))  # Left side box center
            y = int((img_height * 0.10) - (text_height // 2))  # Top area center
            draw.text((x, y), data['doc_number'], fill=(0, 0, 0), font=font_large)
            
            # Add TR license plate number (top right blue plate - white area)
            font_plate = self.get_font(int(88 * min(scale_x, scale_y)))  # Adjusted to match reference image
            bbox = draw.textbbox((0, 0), data['tr_plate'], font=font_plate)
            text_width = bbox[2] - bbox[0]
            # Position in the white area of the TR plate
            x = int((img_width * 0.75) - (text_width // 2))  # Right side white area
            y = int(img_height * 0.08)  # Top area - moved higher up
            draw.text((x, y), data['tr_plate'], fill=(0, 0, 0), font=font_plate)
            
            # Add Arabic name (middle left - light blue box)
            font_arabic = self.get_font(int(85 * min(scale_x, scale_y)), arabic=True)  # Adjusted to match reference image
            ...
            x = int((img_width * 0.26) - (text_width // 2))  # Left side blue box center
            y = int((img_height * 0.44) - (text_height // 2))  # Middle area
            draw.text((x, y), shaped_arabic_name, fill=(0, 0, 0), font=font_arabic)
            
            # Add date/time (middle right - pink box)
            font_date = self.get_font(int(72 * min(scale_x, scale_y)))  # Adjusted to match reference image
            bbox = draw.textbbox((0, 0), data['datetime'], font=font_date)
            text_width = bbox[2] - bbox[0]
            text_height = bbox[3] - bbox[1]
            # Center in the pink box
            x = int((img_width * 0.75) - (text_width // 2))  # Right side pink box center
            ...
            font_irq = self.get_font(int(74 * min(scale_x, scale_y)))  # Adjusted to match reference image
            bbox = draw.textbbox((0, 0), data['irq_plate'], font=font_irq)
            text_width = bbox[2] - bbox[0]
            text_height = bbox[3] - bbox[1]
            # Position in the white area of the IRQ plate
            x = int((img_width * 0.78) - (text_width // 2))  # Right side white area
            y = int((img_height * 0.77) - (text_height // 2))  # Bottom yellow plate area
            draw.text((x, y), data['irq_plate'], fill=(0, 0, 0), font=font_irq)
            
            # Add driver name on truck trailer (Arabic text)
            font_driver = self.get_font(int(80 * min(scale_x, scale_y)), arabic=True)  # Adjusted to match reference image
            ...
            x = int((img_width * 0.32) - (text_width // 2))  # Center of trailer
            y = int((img_height * 0.74) - (text_height // 2))  # On trailer level
            draw.text((x, y), shaped_driver_name, fill=(0, 0, 0), font=font_driver)
    ...
    def create_text_image(self, text: str, max_width: int = 800, font_size: int = 70) -> io.BytesIO:
        ...
        try:
            ...
            for line in lines:
                ...
                draw.text((x_offset, y_offset), shaped_line, font=font, fill=(0, 0, 0, 255))
    ...
    async def text_to_image(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await update.message.reply_photo(
                photo=img_bytes,
                caption=f"🎨 Here's your text as an image!\n\n📝 Original text: {user_text[:50]}{'...' if len(user_text) > 50 else ''}"
            )
            
            logger.info(f"Generated image for user {update.effective_user.id}: {user_text[:50]}...")
